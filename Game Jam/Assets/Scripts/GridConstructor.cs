...
public class GridConstruct
{
    ...
    private int cells;
    public GridConstruct(int _width, int _height, float _cellSize)
    {
        cells = 0;
        this.width = _width;
        this.height = _height;
        ...
        for (int x = 0; x < gridArray.GetLength(0); x++)
            for (int y = 0; y < gridArray.GetLength(1); y++)
            {
                ...
                CreateCellObj(null, GetPos(x, y) + new Vector3(cellSize, cellSize) * 0.5f, cells);
            }
    ...
    public static GameObject CreateCellObj(Transform parent, Vector3 localPos, int cells)
    {
